...
def is_xiaohongshu_url(url):
    """Check if URL is Xiaohongshu (小红书)"""
    parsed = urlparse(url)
    return 'xiaohongshu.com' in parsed.netloc or 'xhs.cn' in parsed.netloc
...
def download_xiaohongshu(url, output_dir, quiet=False):
    """
    Download from Xiaohongshu (小红书)
    Note: This requires proper API access. Returns placeholder for now.
    """
    # 小红书下载需要移动端API或cookie
    # 这里提供基本框架，实际使用需要配置代理或Cookie
    headers = {
    ...
    try:
        # 提取笔记ID
        note_id = None
        patterns = [r'xiaohongshu\.com/explore/([a-zA-Z0-9]+)', r'xhs\.cn/(\w+)']
        ...
        if not note_id:
            return None, "无法提取小红书笔记ID"
        
        # 注意: 小红书API需要代理或Cookie，这里返回提示
        if not quiet:
            print(f"  📕 小红书: 笔记ID={note_id}")
            print(f"  ⚠️  小红书下载需要配置代理或Cookie")
        
        return None, "小红书下载需要配置TikHub API代理 (TODO)"
        
    except Exception as e:
        return None, f"小红书下载失败: {str(e)}"
...
def process_url(url, output_dir, filename=None, max_size=100, timeout=60, 
    ...
    is_xiaohongshu = is_xiaohongshu_url(url)
    ...
    try:
        if is_douyin:
            ...
            # Use Xiaohongshu downloader
            temp_path, error = download_xiaohongshu(url, output_dir, quiet=debug)
            if error:
                return None, error
        elif is_ytdlp:
